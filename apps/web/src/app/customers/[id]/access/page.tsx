...
export default async function AccessPage({
  ...
  const repoRows = await db
    .select({
      ...
      workspaceOwnerId: schema.workspace.ownerId,
    })
    .from(schema.repo)
    .innerJoin(
      schema.workspace,
  ...
  const role = await getUserRole(row.workspaceId, user.id);
  const isLegacyOwner = row.workspaceOwnerId === user.id;
  ...
  const [members, pending, decisions] = await Promise.all([
    listMembers(row.workspaceId),
    listPendingRequestsForWorkspace(row.workspaceId),
    listDecisionsForWorkspace(row.workspaceId),
  ]);
      ...
      <main className="mx-auto max-w-4xl space-y-8 px-4 py-8 sm:px-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <Separator />
            <AccessForms.InviteForm workspaceId={row.workspaceId} />
          </CardContent>
        </Card>
