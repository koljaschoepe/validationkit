...
## Aktive Phase (Juni 2026 →): Genau EIN offener Plan

Nach der 16-Subagent-Plan-Reconciliation (2026-06-18) wurde der alte 10-Bundle-Master `production-launch-readiness` + alle Sub-/Waisen-Pläne nach `docs/plans/done/` archiviert. **`frontend-and-polish` ist seit 2026-06-23 ✅ Done** (→ `done/`; 18 Commits, Blöcke A/B/C/D komplett + E bis auf env-/präferenz-gebundene Deferrals). Es bleibt **exakt ein** aktiver Plan:
- **`docs/plans/production-go-live.md`** — der einzige Plan mit echter User-Aktion: Domain-Kauf, Stripe-Live-KYC, Sentry/Upstash/Resend-Domain provisionieren, Live-Keys, `__Host-`-Cookie + CSP-Enforce, migrate-out-of-build (inkl. der additiven Settings-Migrationen `0020`–`0022` aus dem Frontend-Plan), Impressum-Adressdaten.

Nova-3a ist abgeschlossen (`docs/plans/done/nova-3-repo-polish-and-prod-prep.md`).

**Recently Shipped (Mai–Juni 2026):**
- **Frontend-and-Polish — Settings-Backends + Frontend-Politur** (✅ 2026-06-23) — `docs/plans/done/frontend-and-polish.md`. 4 neue Settings-Backends (API-Keys + read-`/api/v1/scans`, Notification-Preferences mit Email-Enforcement, Outbound-Webhooks Stripe-style-signiert + Inngest-Deliver-Worker, Workspace-Rename/Ownership-Transfer/Account-Profil) hinter 3 **noch anzuwendenden** additiven Migrationen `0020`–`0022`; dazu S20 Inspector→Radix-Dialog, J1 GitHub-Background-Audit, J4 async-Solutions, Inngest-`onFailure`×5, deutsche Email-Templates+Subjects, Saved-Views-URL-State, axe-Harness, vitest-4-Bump. 18 Commits, Gate durchgehend 342 Tests grün / Typecheck 23/23. ⚠️ Offen für User: `pnpm db:migrate` (lokal nach Docker-Start) + visuelles QA.
